...
Number of reads for each of the samples and number of sequences left after sequence assemble and mapping steps.
...
Number of sequences for each of the samples for each downstream filtering and analysis step.

```{r seq_assembled, echo=FALSE, warning=FALSE, results='asis'}
tab_seqs <- read.table("./Table_sequences_assembled.tsv", header=TRUE, sep="\t", check.names = FALSE)

if (any(is.na(tab_seqs$sample_id))) {
  tab_seqs$sample_id <- sapply(tab_seqs$file_0, function(x) unlist(strsplit(as.character(x), "_"))[1])
}

dat <- tidyr::pivot_wider(tab_seqs,id_cols=sample_id,names_from=task, values_from=to_num_seqs)
