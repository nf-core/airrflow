...
process PRESTO_MASKPRIMERS_POSTASSEMBLY {
    ...
    if (params.cprimer_position == "R1") {
        """
        MaskPrimers.py score --nproc ${task.cpus} -s $R1 -p ${cprimers} $primer_start_R1 --maxerror ${params.primer_maxerror} \
        ...
        """
        MaskPrimers.py score --nproc ${task.cpus} -s $R1 -p ${vprimers} $primer_start_R1 --maxerror ${params.primer_maxerror} \
            --mode ${params.primer_mask_mode} --outname ${meta.id}-FWD \
            --log ${meta.id}-FWD.log > "${meta.id}_command_log.txt"
